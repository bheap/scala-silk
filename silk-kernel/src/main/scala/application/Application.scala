package com.bheap.silk.application
...
import com.bheap.silk.actors._
import com.bheap.silk.utils.SilkScout


/** Builds a web application out of a site configuration.
  *
  * For Silk, a site configuration is a directory structure containing views.
  *
...
class Application {
  ...
  def prepareFilterDetails = {
    SilkScout.getFilesInDirectoryOfType("site", "html").toList.map {
      item =>
    ...
    jetty.filter(
      Planify {
        case GET(Path("/reload")) =>
          reload
          ResponseString(<html><body><h1>Silk has reloaded</h1></body></html>.toString)
      }
    )
  }
...
/** Starts up a Jetty Http instance encapsulating our Silk web application. */
object Server {
  def main(args: Array[String]) {
    println("Starting Silk on localhost, port %s" format 8080)
